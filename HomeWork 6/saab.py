...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 26 19:41:08 2019
...
def window_process(samples, kernel_size, stride):
	...
	output_w = (w - kernel_size)//stride + 1;print('h',str(output_h),'w',str(output_w))
	patches = view_as_windows(samples, (1, kernel_size, kernel_size, c), step=(1, stride, stride, c))
	patches = patches.reshape(n, output_h, output_w, c*kernel_size*kernel_size);print('patches shape',patches.shape)
	return patches
...
def multi_Saab_transform(images, labels, kernel_sizes, num_kernels, energy_percent, use_num_images, use_classes):
	...
	for i in range(num_layers):
    	...
    	# Add DC kernel
		num_channels=sample_patches.shape[-1];print(num_channels)
